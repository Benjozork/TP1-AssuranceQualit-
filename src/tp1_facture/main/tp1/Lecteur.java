...
public class Lecteur {
    ...
    public static Commande lireCommande(File fichier) throws IOException {
        ...
        if (!ligneEnteteClients.equals(ENTETE_CLIENTS)) {
            ...
            if (!ligneEntetePlats.equals(ENTETE_PLATS)) {
                ...
                if (!ligneEnteteCommandes.equals(ENTETE_COMMANDES)) {
                    ...
                    lignesCommandes = lireLignesTantQueJusquA(lignesFichier, indLignes,
                            Pattern.compile("^[^\\s]+\\s[^\\s]+\\s\\d+$"), FIN);
        ...
        if (!fichierValide)
            throw new IllegalArgumentException("Le fichier ne respecte pas le format demandé ! ("
                    + raisonfichierInvalide + ", '" + mauvaiseLigne + "')");

        Commande.Client[] clients;
        Commande.Plat[] plats;
        LigneCommande[] commandes;

        clients = Arrays.stream(lignesClients).filter(Objects::nonNull).map(Lecteur::parserClient).distinct()
                .toArray(Commande.Client[]::new);

        plats = Arrays.stream(lignesPlats).filter(Objects::nonNull).map(Lecteur::parserPlat).distinct()
                .toArray(Commande.Plat[]::new);

        commandes = Arrays.stream(lignesCommandes).filter(Objects::nonNull).map(ligne -> {
            LigneCommande lg;
            try {
            ...
            return lg;
        }).filter(Objects::nonNull).distinct().toArray(LigneCommande[]::new);
    ...
    static LigneCommande parserCommande(String ligne, Commande.Client[] clientsDispo, Commande.Plat[] platsDispo) {
        ...
        // lireCommande()
        Commande.Client client = Arrays.stream(clientsDispo).filter(it -> it.nomClient.equals(parties[0])).findFirst()
                .orElse(null);

        // Trouver un plat qui match - si get() échoue on ignore l'erreur dans
        // lireCommande()
        Commande.Plat plat = Arrays.stream(platsDispo).filter(it -> it.nomPlat.equals(parties[1])).findFirst()
                .orElse(null);

        return new LigneCommande(client, plat, parties[2]);
    }
